...
class MongoAdapter(DatabaseAdapter):
    ...
    # Construye la URI de conexión desde las variables de entorno
        connection_string = (
    ...
    def execute_query(self, query: str, params: Optional[Dict] = None) -> List[Dict[str, Any]]:
        """Ejecuta una consulta de MongoDB (query debe ser una cadena JSON o un dict)"""
        import json
        try:
            ...
            if not collection:
                raise ValueError("Debe especificar la colección en los parámetros")
    ...
    def get_all(self, table: str, filters: Optional[Dict] = None) -> List[Dict[str, Any]]:
        ...
        :param table: Nombre de la colección
        :param filters: Diccionario con criterios de filtro
    ...
    def insert_many(self, collection: str, documents: List[Dict[str, Any]]) -> List[str]:
        """Inserta múltiples documentos y retorna sus IDs"""
        col = self.db[collection]
    ...
    def find_many(
        ...
        """Encuentra múltiples documentos con opciones de filtro, proyección y ordenamiento"""
        col = self.db[collection]
        cursor = col.find(filter_query, projection)
        
        if sort:
            sort_list = [(field, ASCENDING if direction == 1 else DESCENDING) 
    ...
    def create_text_index(self, collection: str, fields: List[str]) -> str:
        """Crea un índice de texto para búsquedas"""
        col = self.db[collection]
